...
class BaseSurrogate(object):
    ...
    def _plot_benchmark(self, y, y_pred):
        ...
        for i, ax_ in enumerate(axs):
            ax_[0].plot(y[:,i], label='Targets')
            ax_[0].plot(y_pred[:,i], label='Predictions')
            ax_[1].plot(y_pred[:,i] - y[:,i])
    ...
    def _fit_parallel(self, tasks):
        ...
        result_queue = [pool.apply_async(fit_parallel, t) for t in tasks]
        self.models = [r.get() for r in result_queue]
        pool.close()
        pool.join()
        return None
    ...
    def predict(self, X):
        ...
        return np.reshape(ys, (len(ys), len(ys[0]))).T
...
class RBFN(BaseSurrogate):
    ...
    def __init__(self, n_objs=2, n_models=None, params={}, warm_start=True,
                 n_process=1):
        sup = super(RBFN, self)
        sup.__init__(n_objs=n_objs, n_models=n_models, params=params,
                     n_process=n_process)
