...
from _utils import gaussian_mutator, multiroutine_crossover
from _utils import nsga_crossover, random_crossover
...
theo = load_theo('./ZDT/ZDT6.pf').as_matrix()
...
pop.config_gap_opt(at='least_crowded', radius=0.1, size=2*POP_SIZE,
                   ...
                   crossover_fun=pop.crossover_fun, trial_method='lhs',
...
eq1  = r'$\displaystyle f_1 = 1-exp(-4x_1)sin^6(6\pi x_1)$'
eq2  = r'$\displaystyle f_2 = g(x)h(f_1(x),g(x))$'
eq3 = r'$\displaystyle g(x)=1+9\left[\frac{\sum_{i=2}^{10} x_i}{9}\right]^{0.25}$'
...
ax.scatter(x, y, c='royalblue', s=2.0, label="RBFN Assisted NSGA-II")
...
ax.text(ax.get_xlim()[1]-0.2, 0.8, 'ZDT-6',
