...
def compare(i):
    ...
    path_raw = 'kursawe_moead_mop_raw/' # Path to the new tests with MOPRISM-raw
    path_old = 'kursawe_moead/'         # Path to the previous tests with MOPRISM-G and GOMORS
    path_mop = path_old + str(i) + '/'
    path_gom = path_old + str(i+1) + '/'
    path_mop_raw = path_raw + str(i) + '/'
    path_mop_20  = path_raw + str(i+1) + '/'
    path_mop_10  = path_raw + str(i+2) + '/'
...
def compare_gridplot(i):
    ...
    for ax, f, l, i in zip(axs, fs, labels, range(2*2)):
        ...
        ax.scatter(ref[:, 0], ref[:, 1], s=1., c='black', label="PS", zorder=99)
