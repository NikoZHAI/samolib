...
# -*- coding: utf-8 -*-
"""
Benchmarking MOPRISM-G and GOMORS (with NSGA-II embedded)
...
def run_sim(n, path, simtitle, REVOLUTION, PROBLEM, seed):
    ...
    pop = MOPRISM(size=POP_SIZE, problem=PROBLEM, max_generation=MAX_GENERATION,
                 ...
                 embedded_ea=NSGA2, verbose=VERBOSE, no_improvement_step_tol=3)
...
def main():
    ...
    if not os.path.exists(directory+'seeds.txt'):
        seeds = np.random.randint(0, int(2**32-1), dtype='u4',
                                  size=len(PROBLEM)*len(REVOLUTION))
    ...
    for i in range(len(PROBLEM)*len(REVOLUTION)):
        ...
        pop = run_sim(i, path_, simtitle, r, p(dim=8), s)
        del pop
        # pops.append(pop)
